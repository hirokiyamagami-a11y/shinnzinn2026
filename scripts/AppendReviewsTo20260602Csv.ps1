# AppendReviewsTo20260602Csv.ps1
# 対象: artifacts/review/20260602_レビュー記録票.csv
# 実行内容: バックアップを作成し、NO.2.0 からのレビュー行を既存フォーマットを崩さずに追記する
$repoRoot = Resolve-Path "$PSScriptRoot\.." | Select-Object -ExpandProperty Path
$target = Join-Path $repoRoot 'artifacts\review\20260602_レビュー記録票.csv'
...
# 追記する行（CSVフォーマットを崩さないようダブルクオートで囲む）
$content = @'
